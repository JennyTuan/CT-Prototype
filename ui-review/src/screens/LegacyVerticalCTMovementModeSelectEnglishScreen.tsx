...
const uiFont = 'Arial, "Microsoft YaHei", sans-serif';

type MoveMode = "parallel" | "vertical";

const modeContent: Record<MoveMode, { title: string; description: string }> = {
    parallel: {
        title: "Parallel Movement",
        description: "Scan ring parallel to the base rail",
    },
    vertical: {
        title: "Vertical Movement",
        description: "Scan ring perpendicular to the base rail ",
    },
};
...
export default function LegacyVerticalCTMovementModeSelectEnglishScreen() {
    const [selectedMode, setSelectedMode] = useState<MoveMode>("parallel");

    return (
        <div className="relative h-[768px] w-[1024px] overflow-hidden bg-transparent" style={{ fontFamily: uiFont }}>
            ...
            <section className="absolute left-1/2 top-1/2 w-[560px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[7px] border border-[#8B99B1] bg-[#E5E8EE] shadow-[0_14px_36px_rgba(40,48,64,0.38)]">
                <div className="flex h-[42px] items-center border-b border-[#A8B1C0] bg-[linear-gradient(180deg,#F4F5F7_0%,#D4D9E2_100%)] px-4">
                    <span className="text-[16px] font-semibold text-[#343A45]">Select Movement Mode</span>
                </div>

                <div className="px-7 pb-6 pt-5">
                    <p className="mb-4 text-[14px] text-[#505966]">
                        Confirm the relative positions of the scan ring and base rail, then select the movement mode for this procedure.
                    </p>
                    <div className="flex gap-4">
                        <ModeOption mode="parallel" selected={selectedMode === "parallel"} onClick={() => setSelectedMode("parallel")} />
